...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const plan = id === scenarioContext.ids.recoveryMissionId ? createRecoveryOption(original) : original;
  return NextResponse.json(apiSuccess({ projection: "seed_preview_not_persisted", mission: createMission(plan, id, scenarioContext) }));
}
