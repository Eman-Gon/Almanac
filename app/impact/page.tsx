...
export default function ImpactPage() {
  ...
  return (
    <>
      <PageHeader title="Impact" subtitle={`${recovered ? "Recovery route" : "Strawberry inventory release"} · calculated outcome`} breadcrumbs={[{ label: "Dashboard", href: "/dashboard" }, { label: "Mission", href: `/missions/${missionId}` }, { label: "Impact" }]} backHref={`/missions/${missionId}`} backLabel="Back to Mission" status={<span className="plain-status plain-status-green">Calculated</span>} />
      <div className="page-content impact-page refactor-impact-page">
        ...
        <section className="impact-metric-grid refactor-impact-metric-grid" aria-label="Primary impact metrics">
          <article className="impact-card impact-blue"><Scale aria-hidden="true" /><span>Existing inventory available</span><strong>{productLot.availableQuantityLb.toLocaleString()} lb</strong><small>{productLot.id} · already received at the warehouse</small></article>
          <article className="impact-card impact-green"><CheckCircle2 aria-hidden="true" /><span>Planned outbound before risk deadline</span><strong>{reconciliation.quantityPlannedOutboundInTimeLb.toLocaleString()} lb</strong><small>{(reconciliation.inspectionHoldLb + reconciliation.unallocatedLb).toLocaleString()} lb held or unallocated</small></article>
          <article className="impact-card impact-purple"><UsersRound aria-hidden="true" /><span>Modeled household-equivalents</span><strong>{plan.metrics.modeledHouseholdEquivalents}</strong><small>{scenario.householdWeightLb} lb conversion assumption · not observed households</small></article>
