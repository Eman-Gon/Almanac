...
export function MissionDetailClient({ missionId }: { missionId: "MSN-104" | "MSN-105" }) {
  ...
  return (
    <>
      <PageHeader title="Mission" subtitle={`Strawberry Rescue · ${recovered ? "Recovery route" : "Approved route"}`} breadcrumbs={[{ label: "Dashboard", href: "/dashboard" }, { label: "Missions" }, { label: "Strawberry Rescue" }]} backHref={recovered ? "/packing/PKG-105" : "/packing/PKG-104"} backLabel="Back to Packing Plan" status={<span className={`plain-status ${superseded ? "plain-status-amber" : delivered ? "plain-status-green" : "plain-status-blue"}`}>{mission.status.replaceAll("_", " ")}</span>} />
      <div className="page-content execution-page mission-page refactor-mission-page">
        ...
        <div className="mission-grid refactor-mission-grid">
          <Panel title={recovered ? "Replanned route" : "Approved route"} action={<Link className="panel-link" href={mapHref}><MapPinned size={14} aria-hidden="true" />Open map</Link>} className="mission-map-panel mission-route-panel"><div className="mission-route-preview"><div className="mission-route-preview-icon"><MapPinned size={24} aria-hidden="true" /></div><div><strong>{recovered ? "Replacement route" : "Route ready for execution"}</strong><span>{mission.stops.length} ordered stops · {totalRouteMiles(mission.routeLegs).toFixed(1)} precomputed miles</span><small>The full decision map includes route geometry, capacity, receiving windows, and synchronized stop details.</small></div><Link className="button button-secondary" href={mapHref}>Open route map<ArrowRight size={15} aria-hidden="true" /></Link></div></Panel>
          <Panel title="Ordered stops" className="stops-panel"><ol className="stop-list">{mission.stops.map((stop) => { const complete = stop.status === "complete"; const canceled = stop.status === "canceled"; return <li key={stop.id} className={complete ? "stop-complete" : canceled ? "stop-canceled" : ""}><span className="stop-sequence">{complete ? <CheckCircle2 size={16} aria-hidden="true" /> : canceled ? <AlertTriangle size={16} aria-hidden="true" /> : stop.sequence}</span><div><strong>{stopName(stop.locationId)}</strong><span>{formatWindow(stop.arrivalWindow.start, stop.arrivalWindow.end)}</span><small>{canceled ? `Canceled · ${stop.quantityDropoffLb.toLocaleString()} lb requires recovery` : stop.quantityPickupLb ? `Pick up ${stop.quantityPickupLb.toLocaleString()} lb` : `Drop off ${stop.quantityDropoffLb.toLocaleString()} lb`}</small></div><button className="icon-button" type="button" disabled={!canCompleteStops || nextCompletableStopId !== stop.id || complete || canceled} onClick={() => completeStop(stop.id)} aria-label={`Mark ${stopName(stop.locationId)} complete`}><PackageCheck size={17} aria-hidden="true" /></button></li>; })}</ol></Panel>
        </div>
        ...
        <StickyActionBar status={<><span className="sticky-status-icon">{delivered || recovered ? <CheckCircle2 size={20} aria-hidden="true" /> : <AlertTriangle size={20} aria-hidden="true" />}</span><span><strong>{delivered ? "Mission delivered" : recovered ? "Recovery route ready" : superseded ? "Mission superseded" : canTriggerDisruption ? "Disruption recovery is available" : "Mission execution in progress"}</strong><small>{delivered ? "All active stops are complete; the mission is read-only." : recovered ? "All 1,200 lb remain accounted for." : canTriggerDisruption ? "Changes require a second human approval." : "Complete route stops in order to preserve execution history."}</small></span></>}>
          ...
          {delivered || recovered ? <Link className="button button-primary" href="/impact">View impact<ArrowRight size={16} aria-hidden="true" /></Link> : state.stage === "recovered" ? <Link className="button button-primary" href="/missions/MSN-105">View replacement mission<ArrowRight size={16} aria-hidden="true" /></Link> : state.stage === "disrupted" ? <Link className="button button-danger" href="/simulate">Review recovery<ArrowRight size={16} aria-hidden="true" /></Link> : canTriggerDisruption ? <Link className="button button-primary" href={`/simulate?mission=${missionId}`}>Report disruption<ArrowRight size={16} aria-hidden="true" /></Link> : null}
        </StickyActionBar>
      </div>
    </>
  );
